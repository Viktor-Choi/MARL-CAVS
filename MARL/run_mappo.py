...
def parse_args():
    ...
    parser.add_argument('--model-dir', type=str, required=False,
                        default='./results/Jan-31_07-26-12/models/', help="pretrained model path") # 预训练模型的路径，用于断点续训
...
def train(args):
    ...
    env.config['FLOCKING_REWARD'] = config.getint('ENV_CONFIG', 'FLOCKING_REWARD')
    ...
    env_eval.config['FLOCKING_REWARD'] = config.getint('ENV_CONFIG', 'FLOCKING_REWARD')
    ...
    while mappo.n_episodes < MAX_EPISODES:
        ...
        if mappo.episode_done and ((mappo.n_episodes + 1) % EVAL_INTERVAL == 0):
            rewards, _, _, _ = mappo.evaluation(env_eval, dirs['train_videos'], EVAL_EPISODES)   # evaluation环境通过类方法使用
